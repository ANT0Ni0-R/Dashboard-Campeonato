...
- **Escopo do lancamento (`funilGrupoWhere_`)**: define quais deals do CRM pertencem ao lancamento e e usado em **todas** as queries (base/ativados/vendas/TMR), entao mexer aqui propaga para tudo. Dois cenarios, uma so lever:
  ...
  - Match **APROXIMADO** `LOWER(...) LIKE LOWER(valor)` nos dois campos (nao igualdade) — nomes longos/instaveis, modelo escalavel (duplica a planilha, troca o produto); valor pode vir com `%...%`, e o codigo embrulha se vier sem. Garanta que o trecho case com **um unico** grupo/funil (valide no BQ).
  - **Breakdown por origem** dentro do funil continua via `funilOrigemExpr_` = `COALESCE(origem_do_lead, origin_name, '(sem origem)')`. No FIA, `origem_do_lead` so e preenchido nos deals desse funil (lista de espera / carrinho abandonado / ...), entao o COALESCE da o breakdown de fonte do lead automaticamente.
  - **TMR no canal compartilhado:** no legado o `channel_id` era dedicado e bastava; no FIA o canal TVD e compartilhado, entao o TMR combina **canal + deal no funil (via `contact_id`) + janela**. ~21% das conversas do canal nao cruzam com nenhum deal (sem `contact_id` casavel) e ficam de fora — preco de atribuir a origem pelo deal num canal compartilhado.
  - **Bug ja cometido:** com `=` + valor `%...%` os `%` viravam literais e base/ativados/TMR voltavam vazios — `vendas`/`conversaoGeral` enganavam porque nao dependem desse match (vendas cai em `(sem match)`, conversaoGeral filtra por `campanha`).
